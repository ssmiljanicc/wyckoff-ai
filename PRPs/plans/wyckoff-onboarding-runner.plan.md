...
| Affected systems | Novo: `research/expert-analyses/wiki/`, `scripts/validate_expert_analyses.py`, `scripts/kb_ingest.py`, `config/kb_ingest.yaml`. Pomereno (git mv): `research/expert-analyses/{extracts,by-event,by-structure}/` → `wiki/{...}`. Nedirano: `raw/`, poligon repo (poziva se apsolutnom putanjom, ADR 0006). |
...
| BATCHES.MD ŠEMA | `poligon/knowledge/skills-kb/batches.md:16-33` | kolone mapirane po imenu zaglavlja, "Jedinice" (ne "Issue-i"), CSV stem imena | `batch_column_prefix="jedinic"`, `identity_mode="stem"` |
...
| `research/expert-analyses/wiki/by-event/` | git mv (od `research/expert-analyses/by-event/`) | prazan folder |
| `research/expert-analyses/wiki/by-structure/` | git mv (od `research/expert-analyses/by-structure/`) | prazan folder |
| `research/expert-analyses/wiki/index.md` | create | master index (stari plan Zadatak 7 sadržaj, novi PATH pod `wiki/`) |
...
### Zadatak 1 — Restrukturiraj Faza A skelu pod `wiki/`

- **Action:** `git mv` tri prazna foldera pod novi `wiki/` poddirektorijum
- **Files:** `research/expert-analyses/{extracts,by-event,by-structure}/` → `research/expert-analyses/wiki/{extracts,by-event,by-structure}/`
- **Instruction:** `mkdir -p research/expert-analyses/wiki && git mv research/expert-analyses/extracts research/expert-analyses/wiki/extracts && git mv research/expert-analyses/by-event research/expert-analyses/wiki/by-event && git mv research/expert-analyses/by-structure research/expert-analyses/wiki/by-structure`. Folderi su trenutno prazni (Faza A ih je samo kreirala) — ovo je čist mv, nema sadržaja za merge. `EXTRACT_TEMPLATE.md`, `_progress.md`, `_gaps.md` OSTAJU na `research/expert-analyses/` nivou (ne pod `wiki/`) — mirror skills-kb gde `README.md`/`batches.md`/`run-log.md` sede pored `wiki/`, ne unutra.
- **Pattern:** `poligon/knowledge/skills-kb/` layout (`batches.md`, `README.md`, `run-log.md` pored `wiki/`)
- **Gotchas:** git prazne foldere ne prati — ako `git mv` na praznom folderu ne radi (git ne trekuje prazne direktorijume), umesto toga uradi `mkdir -p research/expert-analyses/wiki/{extracts,by-event,by-structure} && rmdir research/expert-analyses/{extracts,by-event,by-structure}` i dodaj `.gitkeep` u svaki novi prazan folder ako git status ne pokazuje foldere (prazni folder se ne commit-uje bez fajla unutra)
...
- **Instruction:**
  ...
  5. **`collect_findings(kb_root, repo_root, skip_git)`** — SOPSTVENA implementacija (NE `core.collect_findings`), komponuje:
     ...
     - `core.check_frontmatter(pages)` — radi SAMO nad učitanim `pages` = by-event/by-structure/index (extracts nikad ne uđu u `pages` jer nisu u `page_dirs`)
     - `core.check_local_links(pages)`
     - `core.check_index_complete(pages, kb_root)`
...
- **Validation:** `cd ~/projekti/wyckoff-ai && uv run python scripts/validate_expert_analyses.py --kb-root research/expert-analyses --json --skip-git` (mora vratiti validan JSON, `summary.fail == 0` na praznoj skeli — nema extract/by-event fajlova još, tako da provere trivijalno prolaze; `parse_batches` mora vratiti 29 batch-eva)
...
- **Validation:** `cd ~/projekti/wyckoff-ai && uv run python scripts/kb_ingest.py --kb expert-analyses -- --dry-run --skip-git` (mora ispisati tačnu runner komandu bez greške; `--dry-run` je runner-ov postojeći flag, PROVERI da li postoji u `ingest_runner.py` CLI pre oslanjanja na njega — ako ne postoji, koristi `--max-batches 0` ili ekvivalentan bezopasan test)
...
# 4. Validator (Zadatak 5) — PROFILE ugovor + regex + prazan-skela FAIL=0
...
- [ ] `research/expert-analyses/wiki/index.md` postoji sa objašnjenjem dva frontmatter režima
...
- [ ] Validator na praznoj skeli (bez extract/by-event sadržaja) vraća `fail=0`
...
| Runner-ov `--dry-run`/`--max-batches` CLI flag ne postoji tačno pod tim imenom (pretpostavka iz konteksta, nije direktno pročitan CLI parser blok) | Srednja | Nizak | Implementator MORA pročitati `ingest_runner.py` `argparse` sekciju (nije eksplicitno citirana u Mandatory Reading — DOPUNI: pročitaj `def main`/`build_parser`-ekvivalent u `ingest_runner.py` pre Zadatka 6 validacije) da potvrdi tačna imena flagova |
...
- **Sledeći korak**: `/pregled-plana PRPs/plans/wyckoff-onboarding-runner.plan.md` (gate pre implementacije), zatim implementacija istim agentom (`prp-implement` ekvivalent, ili direktno sprovođenje Zadataka 1-7 ovog plana redom).

---
Izvor: orkestrator brief (poligon#221 grupa, Faza 3, mini-PRD `visekorpusni-validirani-ingest.mini-prd.md`) | Generisano: 2026-07-09
