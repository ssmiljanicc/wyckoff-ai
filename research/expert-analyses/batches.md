...
TODO — nije popunjen prompt. Puni tekst disciplinovanog prompta dolazi u SKIL prolazu (izmena-skila/pravljenje-skila nad ovim batches.md, posle wyckoff#89 disciplina rasprave). Kad se popuni, MORA mirror-ovati strukturu B01 prompta (preflight-čitanja, kriterijum validnog para, extract/by-event/by-structure upis, _progress.md/log.md ažuriranje, referenca na "## Disciplina citiranja" za citation-verification + misattribution check, batch-protokol stop-signal) za TAČAN opseg ovog reda (vidi kolonu Jedinice), i TEK TADA se stavi u code-block ili inline-code oblik koji runner-ova funkcija extract_batch_prompt (poligon ingest_runner.py, oko linije 136-167) prepoznaje kao prompt. Namerno VAN oba oblika ovde, bez ijedne obrnute kose crte-navodnika (PR #96 review nalaz): runner tako vraća prazan prompt i odmah označi batch kao blocked ("prompt nije pronađen") BEZ agent poziva — u code-block obliku bi ovaj TODO tekst bio poslat jakom modelu kao pravi prompt i potrošio agent poziv pre guard-a koji proverava da li je nešto stvarno napisano.
